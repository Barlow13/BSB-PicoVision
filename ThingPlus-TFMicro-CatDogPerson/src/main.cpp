 ...
 *    - SSD1306 OLED support with automatic power saving during gating
 */
...
// Toggle between constant inference and gating
// true = constant inference (always run)
// false = allow gating (only capture when PIN17 is HIGH)
#define CONSTANT_INFERENCE false

#if CONSTANT_INFERENCE
#define APP_GATE_BY_PIN17 0
#else
...
static bool init_hardware() {
    stdio_init_all();
    sleep_ms(600); // wait for serial to be ready
    ...
    // Setup monitor pin as input (no pull).
    ...
    while (true) {
        if (queue_try_remove(&q_core0_to_core1, &cmd)) {
            if (cmd == CMD_PROCESS_IMAGE) {
                ...
                uint32_t local_size = g_jpeg_size; (void)local_size;
...
int main() {
    ...
    if (g_pin17_level == 0) {
        if (!waiting_for_pin) {
            printf("[GATE] Waiting for PIN17 HIGH to start capture...\n");
        ...
        PinEvent gev; 
        while (pin_event_buffer_pop(&gev)) {
            printf("[PIN17] %s\n", gev.level ? "HIGH" : "LOW");
        }
        sleep_ms(20);
        continue;
    ...
    if (!capture_jpeg()) { sleep_ms(500); continue; }
        ...
        while (!done && !err) {
            ...
            while (pin_event_buffer_pop(&ev)) {
                printf("[PIN17] %s\n", ev.level ? "HIGH" : "LOW");
                printed_initial = true; (void)printed_initial;
            }
            sleep_ms(8);
        }
        if (!err) {
            ...
            print_result(local);
#ifdef APP_OLED_ENABLE
            bool should_update = local.valid && g_oled.isPowered();
#if APP_GATE_BY_PIN17
            if (g_pin17_level == 0) should_update = false;
#endif
            if (should_update) {
                g_oled.clear();
