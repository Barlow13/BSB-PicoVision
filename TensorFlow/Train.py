...
import os, random, gc, json, argparse
import numpy as np
import pandas as pd
...
CONFIG = {
    ...
    'ALPHA': .35, # MobileNetV2 width multiplier
...
def parse_args():
    ...
    parser.add_argument('--alpha',       type=float, default=None, help='MobileNetV2 width multiplier')
    parser.add_argument('--export-dir',  type=str,   default=None, help='Export directory')
    parser.add_argument('--cutoff',      type=str,   default=None, help='Backbone cutoff layer')
    parser.add_argument('--quant',       type=str,   default=None, help='Quantization mode: int8, float16, or none')
    parser.add_argument('--batch-size',  type=int,   default=None, help='Batch size')
    parser.add_argument('--epochs-frozen',   type=int, default=None, help='Frozen backbone epochs')
    parser.add_argument('--epochs-finetune', type=int, default=None, help='Fine-tune epochs')
...
if __name__ == "__main__":
    ...
    if args.export_dir  is not None: CONFIG['EXPORT_DIR']      = args.export_dir
    if args.cutoff      is not None: CONFIG['BACKBONE_CUTOFF'] = args.cutoff
    if args.quant       is not None: CONFIG['QUANT_MODE']      = args.quant if args.quant != 'none' else None
    if args.batch_size  is not None: CONFIG['BATCH_SIZE']      = args.batch_size
    if args.epochs_frozen   is not None: CONFIG['EPOCHS_FROZEN']   = args.epochs_frozen
    if args.epochs_finetune is not None: CONFIG['EPOCHS_FINETUNE'] = args.epochs_finetune
    CONFIG['CHECKPOINT'] = os.path.join(CONFIG['EXPORT_DIR'], 'best.keras')
    os.makedirs(CONFIG['EXPORT_DIR'], exist_ok=True)
    main()